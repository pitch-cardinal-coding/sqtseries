...
``optimize``, ``backup``, ``vacuum``. Heavy operations run in the handler and
therefore pause ingestion for their duration (a full VACUUM needs exclusive
access anyway); the offline CLI (``sqtseries vacuum``) is the recommended path
for large maintenance.
"""

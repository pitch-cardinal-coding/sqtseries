...
> benchmark harness inserts via `store.insert_many(rows, batch=…)` from
...
> one workstation (2026-08-08) and are for orientation, not guarantees.
...
|------|--------------------:|-----------:|
| 200,000 | ~253,000 | 0.79s |
| 1,000,000 | ~164,000 | 6.1s |
| 2,000,000 (100 series) | ~187,000 | 10.7s |
| 5,000,000 | ~65,000 | 76.9s |
...
|---------|-------|-----|-----|
| 200K (200 pts/series) | range (1h) | 0.15ms | 0.20ms |
| 200K (200 pts/series) | wide agg raw | 0.54ms | 0.68ms |
| 200K (200 pts/series) | wide agg rollup | 0.93ms | 1.82ms |
| 1M (1,000 pts/series) | range (1h) | 0.16ms | 0.24ms |
| 1M (1,000 pts/series) | wide agg raw | 0.96ms | 1.36ms |
| 1M (1,000 pts/series) | wide agg rollup | 0.93ms | 1.17ms |
| 2M (20,000 pts/series) | range (1h) | 0.45ms | 0.77ms |
| 2M (20,000 pts/series) | wide agg raw | 12.0ms | 14.9ms |
| 2M (20,000 pts/series) | wide agg rollup | 1.28ms | 1.55ms |
| 5M (5,000 pts/series) | range (1h) | 0.24ms | 0.39ms |
| 5M (5,000 pts/series) | wide agg raw | 3.91ms | 4.81ms |
| 5M (5,000 pts/series) | wide agg rollup | 1.10ms | 1.85ms |
...
- **The wide aggregate scales linearly with points-per-series when served from
  raw rows** (0.54ms → 3.9ms → 12ms as the series grows 200 → 5,000 → 20,000
  points), because every raw point is streamed to Python and bucketed.
- **The rollup fast path is near-constant** (0.9–1.3ms regardless of how many
  points the series holds): it reads a handful of pre-aggregated hourly rows.
  It wins 3.6–9.4x as soon as the raw scan dominates.
- **Crossover point:** the rollup carries a fixed overhead (~0.4–0.5ms) from
...
- **Where the speed comes from — read the same query twice:** the
  ...
  The 2M rows / 20,000-pts-per-series experiment is the headline: 12.0ms →
  1.28ms (~9x).
